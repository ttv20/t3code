...
interface RightPanelStoreState {
  ...
  /** Session-only user choices. Resource updates and proactive opens do not advance these. */
...
const removeSurfaces = (
  ...
  const removedIds = new Set(surfaceIds);
  const surfaces = current.surfaces.filter((surface) => !removedIds.has(surface.id));
  if (surfaces.length === current.surfaces.length) return current;
  const activeIndex = current.surfaces.findIndex(
    (surface) => surface.id === current.activeSurfaceId,
  );
  const fallback =
    current.surfaces.slice(activeIndex + 1).find((surface) => !removedIds.has(surface.id)) ??
    surfaces.at(-1);
  return {
    ...current,
    ...
    surfaces,
    activeSurfaceId:
      current.activeSurfaceId !== null && removedIds.has(current.activeSurfaceId)
        ? (fallback?.id ?? null)
...
const updateThread = (
  ...
  if (!next.isOpen && next.activeSurfaceId === null && next.surfaces.length === 0) {
    ...
    byThreadKey = { ...byThreadKey, [threadKey]: next };
  }
  if (source === "automatic") return byThreadKey === state.byThreadKey ? state : { byThreadKey };
  return {
    byThreadKey,
...
export const useRightPanelStore = create<RightPanelStoreState>()(
  persist(
    (set, get) => ({
      ...
      openProactive: (ref, surface, expectedUserActionRevision) => {
        ...
        set((state) => {
          ...
          return updateThread(
            state,
            threadKey,
            (current) => upsertSurface(current, surface),
            "automatic",
          );
        });
        return opened;
      ...
      closeTerminal: (ref, surfaceId, terminalId, source = "user") =>
        set((state) =>
          updateThread(
            state,
            scopedThreadKey(ref),
            (current) => {
              ...
              if (terminalIds.length === 0) {
                return removeSurfaces(current, [surfaceId]);
              }
              return {
                ...current,
      ...
      closeSurface: (ref, surfaceId) => get().closeSurfaces(ref, [surfaceId]),
      closeSurfaces: (ref, surfaceIds) =>
        set((state) =>
          updateThread(state, scopedThreadKey(ref), (current) =>
            removeSurfaces(current, surfaceIds),
          ),
        ),
        ...
        set((state) =>
          updateThread(state, scopedThreadKey(ref), (current) => {
            ...
            return removeSurfaces(
              current,
              current.surfaces.slice(index + 1).map((surface) => surface.id),
            );
          }),
        ),
      closeAllSurfaces: (ref) =>
        set((state) =>
          updateThread(state, scopedThreadKey(ref), (current) =>
            removeSurfaces(
              current,
              current.surfaces.map((surface) => surface.id),
            ),
          ),
        ),
